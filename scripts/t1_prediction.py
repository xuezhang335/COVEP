...
def runDeephlapan(task_name, pep1_file, input_hla, outdir):
    ...
    os.system(
        f'conda run -n cov python {cov_dir}/tools/deephlapan/deephlapan/deephlapan.py -F {infile} -O {outdir_tmp}') 
...
def runNetmhcpan(task_name, pep1_file, input_hla, outdir):
    ...
    os.system(
        f'conda run -n cov {cov_dir}/tools/netMHCpan-4.1/netMHCpan -p {infile} -BA -xls -a {str_hla1} -xlsfile {outfile_tmp} > {terminalout}')  # 
